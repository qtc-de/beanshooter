...
public class JMXMPSaslNtlmLifecycleListener implements LifecycleListener
{
    protected int port = 5560;
    protected JMXConnectorServer cs;
    private static final Log log = LogFactory.getLog(JMXMPSaslNtlmLifecycleListener.class);
    ...
    public void lifecycleEvent(final LifecycleEvent event)
    {
        try {
            ...
            if (Lifecycle.START_EVENT == event.getType()) {
                log.info("Starting JMXMP SASL Listener (NTLM)");
                ...
                log.info("Started JMXMP SASL Listener (NTLM) on port " + port);
            }

            else if (Lifecycle.STOP_EVENT == event.getType()) {
                log.info("Stopp JMXMP SASL Listener (NTLM)");
                cs.stop();
                log.info("Stopped JMXMP SASL Listener (NTLM)");
            }
